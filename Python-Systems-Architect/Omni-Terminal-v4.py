...
def core_params_snapshot():
    """
    Minimal hashable parameter snapshot used by the 5-core engine.
    (Keeps caching sane + avoids passing full session_state.)
    """
    return {
        "vec_l": int(st.session_state.vec_l),
        "vol_n": int(st.session_state.vol_n),
        "vec_sm": int(st.session_state.vec_sm),
        "vec_super": float(st.session_state.vec_super),
        "vec_resist": float(st.session_state.vec_resist),
        "vec_strict": float(st.session_state.vec_strict),
        "br_l": int(st.session_state.br_l),
        "br_m": float(st.session_state.br_m),
        "br_th": float(st.session_state.br_th),
        "smc_l": int(st.session_state.smc_l),
    }
...
with st.sidebar:
    ...
    with st.expander("🌍 Feed Configuration", expanded=True):
        ...
        # Watchlist / Scout controls (X-Factor)
        st.session_state.watchlist = st.text_area(
            "Watchlist (comma-separated CCXT symbols)",
            st.session_state.watchlist,
            height=70
        )
        st.session_state.scout_tf = st.selectbox(
            "Scout Interval",
            tf_list,
            index=tf_list.index(st.session_state.scout_tf) if st.session_state.scout_tf in tf_list else 2
        )
...
# 7. PROCESSING
# ==========================================
# Cache-bust bucket that actually changes over time (fixes Auto-Pilot "sticking")
ttl_seconds = TF_TTL.get(st.session_state.tf, 30)
ttl_bucket = int(time_lib.time() // ttl_seconds)
...
def should_alert(signal_name: str, candle_ts: pd.Timestamp) -> bool:
    # Dedupe must be per symbol + tf + signal (prevents cross-signal suppression)
    key = f"{resolved_sym}|{st.session_state.tf}|{signal_name}"
    ts = str(candle_ts)
...
t1, t2, t3, t4, t5, t6, t7 = st.tabs(["🧠 Brain", "🏛️ Structure", "⚡ Vector", "⚛️ Quantum", "📺 TV View", "🤖 AI Council", "🛰️ Scout"])
...
def bias_score(row: dict) -> int:
    ...
    + smc_buy/sell (+1/-1)
    + rqzo sign (+1/-1)
    gate SAFE amplifies directional score by +1 magnitude (if non-zero).
    """
    s = 0
    s += int(row.get("vec_state", 0))
    s += int(row.get("br_trend", 0))
    s += int(row.get("mat_sig", 0))
    if bool(row.get("smc_buy", False)):
        s += 1
    if bool(row.get("smc_sell", False)):
        s -= 1
    rq = float(row.get("rqzo", 0.0))
    s += (1 if rq > 1 else (-1 if rq < -1 else 0))
    if bool(row.get("gate", False)) and s != 0:
        s = int(np.sign(s) * (abs(s) + 1))
    return int(s)

@st.cache_data(show_spinner=False)
def scout_symbol(exch_name: str, sym: str, tf: str, lim: int, _bucket: int, p: dict):
    df0, resolved, err = fetch_ohlcv(exch_name, sym, tf, lim, _bucket)
    if df0.empty:
        return None, resolved, err

    # Run the same 5-core chain with a minimal param snapshot
    df1 = calc_vector(df0, p)
    df1 = calc_brain(df1, p)
    df1 = calc_rqzo(df1, p)
    df1 = calc_matrix(df1, p)
    df1 = calc_smc(df1, p)

    last_row = df1.iloc[-1].to_dict()
    last_row["timestamp"] = str(df1.iloc[-1]["timestamp"])
    last_row["symbol"] = resolved
    last_row["score"] = bias_score(last_row)
    return last_row, resolved, ""
...
# ---- Scout (X-Factor)
with t7:
    st.markdown("### 🛰️ Signal Constellation Scout")
    syms = parse_watchlist(st.session_state.watchlist)

    cA, cB, cC = st.columns([1, 1, 2])
    with cA:
        scout_lim = st.slider("Scout Candles", 200, 1200, 420, step=20)
    with cB:
        run_scout = st.button("🔭 SCAN WATCHLIST", use_container_width=True)
    with cC:
        st.caption("Scans your watchlist using the *same* 5-core engine, then ranks by a composite score. Click to load into main terminal.")

    if run_scout:
        if not syms:
            st.warning("Watchlist is empty.")
        else:
            ttl_seconds_s = TF_TTL.get(st.session_state.scout_tf, 30)
            bucket_s = int(time_lib.time() // ttl_seconds_s)
            p = core_params_snapshot()

            rows = []
            prog = st.progress(0)
            for i, s in enumerate(syms, start=1):
                row, resolved, err = scout_symbol(
                    st.session_state.exch,
                    s,
                    st.session_state.scout_tf,
                    int(scout_lim),
                    bucket_s,
                    p
                )
                if row is None:
                    rows.append({
                        "symbol": s,
                        "score": None,
                        "status": f"VOID: {err[:120]}" if err else "VOID"
                    })
                else:
                    rows.append({
                        "symbol": row["symbol"],
                        ...
                        "flux": float(row["flux"]),
                        "vec_state": int(row["vec_state"]),
                        "br_trend": int(row["br_trend"]),
                        "gate": bool(row["gate"]),
                        "mat_sig": int(row["mat_sig"]),
                        "smc_buy": bool(row["smc_buy"]),
                        "smc_sell": bool(row["smc_sell"]),
                        "rqzo": float(row["rqzo"]),
                        "status": "OK"
                    })
            ...
            if "score" in scout_df.columns:
                scout_df = scout_df.sort_values(by="score", ascending=False, na_position="last")

            st.session_state.scout_df = scout_df
            st.success("Scout scan complete.")

    scout_df = st.session_state.scout_df
    if isinstance(scout_df, pd.DataFrame) and not scout_df.empty:
        st.dataframe(scout_df, use_container_width=True, height=420)

        top = scout_df[scout_df.get("status") == "OK"].head(10)
        if not top.empty:
            pick = st.selectbox("Load symbol into main terminal", top["symbol"].tolist())
            col1, col2 = st.columns([1, 2])
            with col1:
                if st.button("⚡ LOAD INTO TERMINAL", use_container_width=True):
                    st.session_state.sym = pick
                    fetch_ohlcv.clear()
                    st.rerun()
            with col2:
                st.caption("Loads selected scout symbol into the main terminal (same exchange + your main TF).")

        csv = scout_df.to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download Scout CSV", csv, file_name="pentagram_scout.csv", mime="text/csv")
