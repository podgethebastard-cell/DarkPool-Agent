...
def load_universe(universe_key: str):
    ...
    # --- Crypto: CoinGecko Top 100 -> SYMBOL-USD ---
...
# =============================================================================
# 5) yfinance FETCH — optimized throttle handling (backoff + salvage)
# =============================================================================
...
def _extract_close_df(data, tickers):
    ...
    if isinstance(data.columns, pd.MultiIndex):
        ...
        if "Close" in data:
            # last resort: single column close
...
def get_market_close_robust(tickers_list, period="1y", interval="1d", chunk_size=60, max_retries=4):
    ...
    for ch in chunks:
        ...
        # identify missing in chunk
        chunk_missing = []
        if close_df is None or close_df.empty:
            chunk_missing = ch[:]  # all missing if chunk failed
        else:
            for t in ch:
                if t not in close_df.columns or close_df[t].dropna().empty:
                    chunk_missing.append(t)

        # salvage missing symbols individually with escalating backoff
        salvage_cols = {}
        if chunk_missing:
...
# =============================================================================
# 7) REGIME TIMELINE + SELECTED-ASSET HIGHLIGHT
# =============================================================================
...
# =============================================================================
# 8) APEX SMC v8 — OPTIMIZED CORE (NumPy loops for stateful parts)
# =============================================================================
...
def apex_smc_v8(df: pd.DataFrame, cfg: dict):
    ...
    # Trend state machine (NumPy loop)
    close_np = df["Close"].to_numpy(dtype=float)
    upper_np = upper.to_numpy(dtype=float)
    lower_np = lower.to_numpy(dtype=float)
    ...
    for i in range(1, len(df)):
        last = trend_np[i-1]
        confirmed = 1 if close_np[i] > upper_np[i] else (-1 if close_np[i] < lower_np[i] else last)
    ...
    trail_atr = (atr(df, 14) * 2.0).to_numpy(dtype=float)
    ...
    # Pivots + zones on capped window (faithful; expensive part bounded)
...
# =============================================================================
# 9) CLICK-ANYWHERE PANES (component + fallback)
# =============================================================================
...
def pane_click_figure(label: str, ticker: str, desc: str, val: float, pct: float):
    ...
    # 1x1 heatmap: clickable everywhere
...
# =============================================================================
# 11) BUILD TICKER SET (Curated + ratios + crypto components + optional universe)
# =============================================================================
...
# =============================================================================
# 14) MARKETS TAB — TRUE TAP-ANYWHERE PANES + SEARCH LINK
# =============================================================================
with tab1:
    ...
    for i, (label, (ticker, desc)) in enumerate(items.items()):
        with cols[i % 2]:
            ...
            # Tap-anywhere pane if component available
            if plotly_events is not None:
                ...
                if clicked:
                    # any click returns at least one point
                    set_selected_asset(label, ticker)
                    st.success(f"Selected: {label} ({ticker})")
            else:
                # Fallback (still big-touch), no omission
...
# =============================================================================
# 16) INTEL TAB — TIMELINE + SELECTED ASSET SPOTLIGHT (highlight integrated)
# =============================================================================
with tab3:
    ...
    if timeline_df is None or timeline_df.empty:
        ...
        # highlight the selected asset "now" by vertical marker at last available regime date
...
with tab4:
    ...
    with st.spinner("Fetching OHLCV + computing Apex SMC v8..."):
        ...
        if ohlc is None or ohlc.empty:
            ...
            # performance cap (keeps pivots + zones bounded)
