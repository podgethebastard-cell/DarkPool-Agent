# app.py — THE PENTAGRAM | Omni-Terminal (Full Updated Complete Code)
# Features:
# - Scanner Mode (watchlist import + flux spike leaderboard across 20–100+ symbols)
# - Auto-record top spikes as Signal Events -> Replay becomes a “market radar timeline”
# - MTF Consensus Panel (15m/1h/4h alignment score)
# - Signal Replay (click any event -> rebuild candle context + optional AI “why”)
# - Backtest-lite (last N signals -> hit-rate + MFE/MAE + returns)
...
<style>
    ...
    .chip {
        display:inline-block; padding:2px 8px; border:1px solid #222; border-radius:999px;
...
def calc_brain(df: pd.DataFrame, p: Dict) -> pd.DataFrame:
    ...
    # ATR-like (kept from your intent, hardened)
...
def resolve_symbol(ex, sym: str) -> str:
    ...
    if "/" not in sym:
        # attempt split by common quote lengths
...
def latest_core_state(exch: str, symbol: str, tf: str, lim: int, params: Dict):
    df, sym2, err = fetch_ohlcv(exch, symbol, tf, lim, 999)
    if df.empty:
...
# ==========================================
# 5) WATCHLIST + SCANNER + CONSENSUS + REPLAY + BACKTEST STRUCTURES
# ==========================================
...
def tv_token_to_ccxt_symbol(token: str, quote_priority: List[str] = None) -> str:
    quote_priority = quote_priority or COMMON_QUOTES
    t = token.strip().upper()
...
def run_flux_scanner(exch_name: str, symbols: List[str], tf: str, lim: int, params: Dict,
                     spike_window: int = 120) -> pd.DataFrame:
    ex = get_exchange(exch_name)
    ...
    for i, sym in enumerate(symbols):
        try:
            df, sym2, err = fetch_ohlcv(exch_name, sym, tf, lim, 999)
            if df.empty:
                raise RuntimeError(err or "fetch failed")
        ...
        prog.progress((i + 1) / n, text=f"Scanning {i+1}/{n}: {sym}")
        time_lib.sleep(sleep_s)

    prog.empty()
    df_out = pd.DataFrame(rows)
    df_out["abs_z"] = df_out["flux_z"].abs()
    df_out = df_out.sort_values(["abs_z"], ascending=False, na_position="last").reset_index(drop=True)
    return df_out
...
def consensus_alignment(exch: str, symbol: str, tfs: List[str], lim: int, params: Dict):
    ...
    dfp["dir"] = dfp.apply(lambda r: direction_from_row(r), axis=1)
...
def backtest_lite(df: pd.DataFrame, signal_col: str, direction: int,
                  last_n: int = 50, horizon: int = 30,
                  tp_pct: float = 0.8, sl_pct: float = 0.5) -> pd.DataFrame:
...
# ==========================================
# 6) SIGNAL EVENTS + RADAR AUTO-RECORD
# ==========================================
@dataclass
class SignalEvent:
    id: str
    ...
    smc_buy: bool
    smc_sell: bool
    flux_z: float = 0.0  # for radar spikes
...
def init():
    defaults = {
        ...
        "radar_last_record_ts": {},  # "EXCH|SYM|TF" -> ts
    }
...
def log_event(msg: str):
    ...
    st.session_state.signal_log = ([{"ts": ts, "msg": msg}] + st.session_state.signal_log)[:250]
...
def record_signal_event(exch: str, symbol: str, tf: str, signal: str, last_row: pd.Series, flux_z: float = 0.0):
    uid = f"{exch}|{symbol}|{tf}|{signal}|{last_row['timestamp']}"
    ev = SignalEvent(
        id=uid,
    ...
    if not any(x.id == uid for x in st.session_state.signal_history):
        st.session_state.signal_history = [ev] + st.session_state.signal_history
        st.session_state.signal_history = st.session_state.signal_history[:800]

def record_radar_spike_event(exch: str, symbol: str, tf: str, row: dict):
    ts = row.get("ts", "")
    if not ts:
        return
    ...
    signal_name = f"radar_spike_{direction}"

    uid = f"{exch}|{symbol}|{tf}|{signal_name}|{ts}"
    ev = SignalEvent(
        id=uid,
        ts=ts,
        exch=exch,
...
def auto_record_top_spikes(scan_df: pd.DataFrame, exch: str, tf: str,
                           top_k: int = 15,
                           min_abs_z: float = 2.0,
                           only_gate_safe: bool = False,
                           require_no_error: bool = True):
    if scan_df is None or scan_df.empty:
        return
    df = scan_df.copy()
    if require_no_error and "error" in df.columns:
        df = df[df["error"].isna()]
    ...
    for _, r in df.iterrows():
        record_radar_spike_event(exch, str(r["symbol"]), tf, r.to_dict())
...
with st.sidebar:
    ...
    with st.expander("🌍 Feed Configuration", expanded=True):
        ...
        st.session_state.lim = st.slider("Candles", 200, 2500, int(st.session_state.lim), step=50)
...
# ==========================================
# 9) MAIN SYMBOL PROCESSING (for core tabs + alerts)
# ==========================================
...
# ==========================================
# 10) ALERT ENGINE (deduped) + record_signal_event
# ==========================================
events = []
...
for sig, msg in events:
    if should_alert(sig, last["timestamp"]):
        # record event into replay history
        record_signal_event(st.session_state.exch, resolved_sym, st.session_state.tf, sig, last, flux_z=0.0)
...
# =========================
# TAB: SCANNER (with radar auto-record)
# =========================
with t_scanner:
    st.markdown("## 📡 Scanner Mode — Flux Spike Leaderboard")
    st.caption("Import a watchlist (TradingView export TXT or pasted tickers). Run scan. Optionally auto-record top spikes into Replay timeline.")
    ...
    with c1:
        ...
        raw_text = st.text_area(
            "Or paste symbols here (comma or newline separated)",
            value=raw_text,
            height=120,
    ...
    with c2:
        ...
        scan_tf = st.selectbox("Scan timeframe", ["1m", "5m", "15m", "1h", "4h", "1d"], index=2)
        lim = st.slider("Candles per symbol", 150, 800, 260, step=10)
        spike_win = st.slider("Spike window (flux)", 60, 240, 120, step=10)
        topn = st.slider("Show Top N", 10, 100, 30, step=5)

    tokens = parse_watchlist_text(raw_text)
    if tokens:
        resolved, rejected = normalize_watchlist_for_exchange(ex, tokens, quote_priority=quote_priority)
        st.session_state.watchlist_tokens = tokens
        st.session_state.watchlist_symbols = resolved
        st.session_state.watchlist_rejected = rejected

        st.caption(f"Tokens parsed: {len(tokens)} · Resolved on {st.session_state.exch}: {len(resolved)} · Rejected: {len(rejected)}")
        if rejected:
    ...
    if len(st.session_state.watchlist_symbols) < 5:
        st.warning("Resolved watchlist is very small. If your TradingView list contains mixed exchanges, choose one exchange in Streamlit and/or paste CCXT-format symbols like BTC/USDT.")
        st.stop()

    st.markdown("### Radar Timeline (auto-record spikes)")
    cA, cB, cC, cD = st.columns(4)
    with cA:
        radar_on = st.toggle("📌 Auto-record spikes", value=True)
    with cB:
        radar_topk = st.slider("Top K per scan", 5, 50, 15, step=5)
    with cC:
        radar_minz = st.slider("Min |z| threshold", 0.5, 6.0, 2.0, step=0.1)
    with cD:
        radar_safe_only = st.toggle("SAFE only (gate)", value=False)

    if st.button("🔍 RUN FLUX SCAN", type="primary", use_container_width=True):
        results = run_flux_scanner(
            st.session_state.exch,
            st.session_state.watchlist_symbols[:200],
            scan_tf,
            lim,
            st.session_state,
        ...
        if radar_on:
            auto_record_top_spikes(
                ...
                tf=scan_tf,
                top_k=radar_topk,
                min_abs_z=radar_minz,
                only_gate_safe=radar_safe_only,
                require_no_error=True
            )
            log_event(f"📡 Radar recorded top spikes: K={radar_topk}  min|z|={radar_minz:.1f}  tf={scan_tf}")

    if st.session_state.scan_results is not None:
        df_scan = st.session_state.scan_results.copy()
        st.markdown("### Leaderboard (sorted by |flux_z|)")
        ...
        if valid_syms:
            pick = st.selectbox("Select a symbol from leaderboard", valid_syms)
            if st.button("Set as main chart symbol", use_container_width=True):
                st.session_state.sym = pick
                st.success(f"Main symbol set: {pick}")
        else:
            st.info("No valid symbols in this scan output (check exchange/symbol formats).")
...
with t_consensus:
    ...
    if "_consensus_df" in st.session_state:
        ...
        st.caption("dir = blended directional verdict per TF (gate damped). Alignment is weighted (15m 0.25 / 1h 0.35 / 4h 0.40).")


# =========================
# TAB: SIGNAL REPLAY (Radar Timeline)
# =========================
with t_replay:
    st.markdown("## 🎞 Signal Replay — Market Radar Timeline")

    if not st.session_state.signal_history:
        st.info("No recorded signals yet. Trigger alerts or run Scanner with auto-record enabled.")
        st.stop()

    events = st.session_state.signal_history

    # Sort radar spikes first by abs z, then others
    events_sorted = sorted(
        events,
        key=lambda e: (0 if str(e.signal).startswith("radar_spike") else 1, -(abs(getattr(e, "flux_z", 0.0))))
    )

    options = [
        f"{ev.ts} | {ev.symbol} | {ev.tf} | {ev.signal} | z={getattr(ev,'flux_z',0.0):.2f}"
        for ev in events_sorted
    ]
    idx = st.selectbox("Pick a signal", list(range(len(options))), format_func=lambda i: options[i])
    ev = events_sorted[idx]

    st.markdown(
        f"### Selected: `{ev.symbol}` · `{ev.tf}` · `{ev.signal}` · `{ev.ts}`"
    )
    chips = []
    if str(ev.signal).startswith("radar_spike"):
        chips.append(f"Radar z={ev.flux_z:.2f}")
    chips.append(f"Flux={ev.flux:.3f}")
    chips.append(f"Brain={ev.brain_trend}")
    chips.append("SAFE" if ev.gate_safe else "CHAOS")
    chips.append(f"Matrix={ev.mat_sig}")
    if ev.smc_buy: chips.append("SMC BUY")
    if ev.smc_sell: chips.append("SMC SELL")
    ...
    if st.button("▶️ REPLAY CONTEXT", type="primary", use_container_width=True):
        try:
            ...
            why = []
            why.append(f"Flux={ev.flux:.3f}")
            if str(ev.signal).startswith("radar_spike"):
                why.append(f"SpikeZ={ev.flux_z:.2f}")
            why.append(f"BrainTrend={ev.brain_trend}")
            why.append(f"GateSafe={ev.gate_safe}")
            why.append(f"MatrixSig={ev.mat_sig}")
            if ev.smc_buy: why.append("SMC=BUY")
            if ev.smc_sell: why.append("SMC=SELL")
...
- ts={ev.ts}
- signal={ev.signal}
- entry_price={ev.price:.6f}

State:
- flux={ev.flux:.6f}
- flux_z={ev.flux_z:.4f}
- brain_trend={ev.brain_trend}
- gate_safe={ev.gate_safe}
- mat_sig={ev.mat_sig}
- smc_buy={ev.smc_buy}
- smc_sell={ev.smc_sell}
...
""".strip()
    ...
    for row in st.session_state.signal_log[:80]:
        st.markdown(
...
""".strip()
    ...
    st.markdown("### Debug Footer")
    if st.session_state.debug:
        st.caption(f"Resolved Symbol: {resolved_sym} · Candles: {len(df_main)} · Last candle: {last['timestamp']}")
        st.caption(f"OpenAI Available: {OPENAI_AVAILABLE} · Tweepy Available: {TWEEPY_AVAILABLE} · Autorefresh Available: {AUTOR_AVAILABLE}")
