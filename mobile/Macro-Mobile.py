...
def generate_ai_analysis(summary_text):
    try:
        ...
        if not api_key: return "⚠️ Missing OpenAI API Key in st.secrets."
...
# --- 6. APP NAVIGATION (Mobile Tabs) ---
# Replaced Sidebar Radio with Tabs for Mobile Optimization
...
with tab1:
    # Use selectbox for categories (better than radio on mobile)
    selected_category = st.selectbox("Asset Class", list(TICKERS.keys()))
    
    items = TICKERS[selected_category]
    
    # 2 Columns allows metrics to breathe on mobile, 3 is too tight
    cols = st.columns(2)
