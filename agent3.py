...
# ==========================================
# 1.1 WORLD CLOCK WIDGET (REPAIRED)
# ==========================================
st.markdown("""
<div style="
    display: flex;
...
">
    <div style="text-align: center;">
        ...
        <div id="clock-ny" style="color: #2979FF; font-weight: bold; font-size: 1.1rem;">00:00:00</div>
    </div>
    <div style="width: 1px; height: 30px; background: #333;"></div>
    <div style="text-align: center;">
        <div style="color: #888; font-size: 0.75rem; letter-spacing: 1px;">LONDON</div>
        <div id="clock-uk" style="color: #E040FB; font-weight: bold; font-size: 1.1rem;">00:00:00</div>
    </div>
    <div style="width: 1px; height: 30px; background: #333;"></div>
    <div style="text-align: center;">
        <div style="color: #888; font-size: 0.75rem; letter-spacing: 1px;">TOKYO</div>
        <div id="clock-jp" style="color: #00E676; font-weight: bold; font-size: 1.1rem;">00:00:00</div>
    </div>
</div>
...
function updateTime() {
    const now = new Date();
    const ny = new Intl.DateTimeFormat('en-US', { timeZone: 'America/New_York', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now);
    const uk = new Intl.DateTimeFormat('en-GB', { timeZone: 'Europe/London', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now);
    const jp = new Intl.DateTimeFormat('ja-JP', { timeZone: 'Asia/Tokyo', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now);

    document.getElementById('clock-ny').innerText = ny;
    document.getElementById('clock-uk').innerText = uk;
    document.getElementById('clock-jp').innerText = jp;
}
...
def calculate_engine(df, ticker):
    ...
    rsi_mf = 100 - (100 / (1 + (avg_gain_mf / avg_loss_mf)))
    ...
    rmf = tp * df['Volume']
    mfi_ratio = pd.Series(np.where(tp > tp.shift(1), rmf, 0)).rolling(14).sum() / pd.Series(np.where(tp < tp.shift(1), rmf, 0)).rolling(14).sum()
    df['MFI'] = 100 - (100 / (1 + mfi_ratio))

    up = df['High'].diff()
    down = -df['Low'].diff()
    tr = calculate_atr(df, 14)
    p_di = 100 * calculate_rma(pd.Series(np.where((up > down) & (up > 0), up, 0)), 14) / tr
    m_di = 100 * calculate_rma(pd.Series(np.where((down > up) & (down > 0), down, 0)), 14) / tr
...
if df is not None and not df.empty:
    ...
    # Corrected column reference: atr_val points to calculation used in Trade Plans
    atr_val = last['Apex_ATR']
    ...
    tabs = st.tabs(["🌊 Apex Master", "💀 DarkPool Trends", "🔋 Money Flow Matrix", "☁️ Ichimoku", "📈 Oscillators", "🤖 AI Analyst", "📡 Broadcast"])
    ...
    with tabs[2]:
        ...
        fm.add_trace(go.Bar(x=df.index, y=df['Matrix_MF'], marker_color=['#00E676' if v > 0 else '#FF1744' for v in df['Matrix_MF']]), row=1, col=1)
        fm.add_trace(go.Scatter(x=df.index, y=df['Matrix_HyperWave'], line=dict(color='white')), row=1, col=1)
        fm.add_trace(go.Bar(x=df.index, y=df['Hist'], marker_color='cyan'), row=2, col=1)
        fm.add_trace(go.Bar(x=df.index, y=df['Vol_Delta'], marker_color=['#00E676' if v > 0 else '#FF1744' for v in df['Vol_Delta']]), row=3, col=1)
        fm.update_layout(height=600, template="plotly_dark")
    ...
    with tabs[5]:
        if st.button("Generate Alpha Report"):
            if api_key:
                ...
                prompt = f"Analyze {ticker} at ${last['Close']:.2f}. Trend: {'BULL' if last['Apex_Trend']==1 else 'BEAR'}. RVOL: {last['RVOL']:.2f}. Divergence: {last['Div_Signal']}. Plan trades with 2.0x ATR stops."
