...
class TradingAgent:
    ...
    def run(self):
        ...
        steps = 0
        total_steps = self.cursor._max_index + 1
        ...
        while self.cursor.has_next():
            ...
            self.portfolio.update_equity(tick['close'])
            
            # Update progress every 10 ticks to speed up UI
            steps += 1
            if steps % 10 == 0:
                progress_bar.progress(min(steps / total_steps, 1.0))
                status_text.text(f"Processing candle {steps}/{total_steps}...")
...
def main():
    ...
    with st.sidebar:
        ...
        long_window = st.slider("Long MA Window", 20, 100, 50)
        data_points = st.slider("Simulation Duration (Candles)", 100, 2000, 500)
        
        run_btn = st.button("Run Backtest", type="primary")

    # Main Logic
    if run_btn:
        with st.spinner("Generating Market Data & Running Simulation..."):
            # 1. Setup Data
            ...
            col1.metric("Final Equity", f"${final_equity:,.2f}", f"{roi:.2f}%")
            col2.metric("Total Trades", len(portfolio.trade_log))
            
            # Calculate Win Rate
            exits = [t for t in portfolio.trade_log if t.side == 'SELL']
            win_rate = 0
            if exits:
            ...
            if portfolio.trade_log:
                df_log = pd.DataFrame(portfolio.trade_log)
                st.dataframe(df_log)
            else:
