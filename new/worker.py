```python
# app.py
# Personal AI Assistant Dashboard — GPT-5.2 Council (single-file, no emojis)
#
# Requirements:
#   pip install streamlit requests pypdf python-dotenv
#
# Run:
...
def init_page() -> None:
    ...
    st.title(APP_TITLE)
    st.caption("4 analyst council dashboard. GPT-5.2 optimized (Responses API by default for gpt-5* / o* models).")
...
def sidebar_settings(cfg: LLMConfig) -> LLMConfig:
    ...
    api_mode = st.sidebar.selectbox(
        ...
        options=["auto", "responses", "chat"],
        index=["auto", "responses", "chat"].index(cfg.api_mode) if cfg.api_mode in ["auto", "responses", "chat"] else 0,
        help="auto = Responses for gpt-5* & o* models, else Chat Completions. You can force either.",
    )
...
def render_council_tab(llm: LLMClient, cfg: LLMConfig, state: AppState, analysts: Dict[str, Analyst]) -> None:
    ...
    with cols[1]:
        ...
        include_decision_card = st.checkbox("Generate Council Decision Card (X-Factor)", value=True)
        ...
        if include_decision_card:
            with st.spinner("Synthesizing Decision Card…"):
                ...
                try:
                    ...
                    state.last_decision_card = f"⚠️ LLM error: {e}"
        else:
            state.last_decision_card = ""
...
def main() -> None:
    ...
    tabs = st.tabs(["🧠 Council", "💬 Single Analyst", "ℹ️ About"])
    with tabs[0]:
        render_council_tab(llm, cfg, state, analysts)
    with tabs[1]:
        render_single_tab(llm, cfg, state, analysts)
