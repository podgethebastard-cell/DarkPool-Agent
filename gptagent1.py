...
Fixed Version: Resolved HTTP 451 Geo-Blocking Issues & Added Region Selector
"""
import time
import math
import sqlite3
...
# Available API Endpoints to bypass Geo-blocking
API_DOMAINS = {
    "Binance Global (Default)": "https://api.binance.com/api/v3",
    "Binance US (USA Users)": "https://api.binance.us/api/v3",
    "Binance Vision (Public Data/Backup)": "https://data-api.binance.vision/api/v3"
}
...
# --- NEW: API Region Selector ---
api_choice = st.sidebar.selectbox(
    "API Region (Fix 451 Errors)",
    options=list(API_DOMAINS.keys()),
    index=0,
    help="If you get a 451 error, switch this setting."
)
BINANCE_API_BASE = API_DOMAINS[api_choice]
# -------------------------------
...
def get_db_connection(path: str = DB_PATH):
    ...
    # Added timeout to prevent locking
...
def get_klines(
    ...
    Now accepts base_url to handle geo-blocking
    """
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # Handle Geo-blocking (451) specifically
            if e.response.status_code == 451:
                st.error(f"⛔ GEO-BLOCK DETECTED (Error 451). \n\nYour current IP is blocked by {base_url}. \nPlease select a different 'API Region' in the Sidebar.")
                return pd.DataFrame()
...
with col_left:
    ...
    with st.spinner("Fetching market data from Binance..."):
        # PASSED API_DOMAINS[api_choice] to handle Geo-blocking
        df = get_klines(BINANCE_API_BASE, symbol, timeframe, limit)
...
---
**Version:** 2.1.0 (Fixed Geo-Blocking)  
