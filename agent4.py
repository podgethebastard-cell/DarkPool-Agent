...
st.markdown("""
    ...
    .titan-card h4 { margin: 0; font-size: 0.8rem; color: #888; text-transform: uppercase; letter-spacing: 1px; }
...
# 2. SIDEBAR CONTROLS
# ==========================================
st.sidebar.header("⚡ TITAN SCALPER")
st.sidebar.markdown("<span style='color:#666; font-size:0.8em'>SYSTEM STATUS: ONLINE</span>", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
tg_on = st.sidebar.checkbox("Active", value=False)
bot_token = st.sidebar.text_input("Bot Token", type="password")
chat_id = st.sidebar.text_input("Chat ID")
test_btn = st.sidebar.button("Test Connection")
...
def send_telegram_msg(token, chat, msg):
    ...
    except Exception as e: st.error(f"TG Error: {e}")
...
def render_tv(sym):
    ...
    <script>new TradingView.widget({{
      "width": "100%", "height": 400, "symbol": "{s}", "interval": "5",
...
#DarkPool #Titan #Crypto"""
    ...
    with c1:
        ...
        <div class="titan-card {trend_cls}">
            <h4>Current Price</h4>
            <h2>${last['close']:,.2f}</h2>
            <div class="sub">Trend: <span class="{trend_txt_cls}"><b>{trend_lbl}</b></span></div>
    ...
    with c2:
        ...
        <div class="titan-card {trend_cls}">
            ...
            <div class="sub">Risk Mgmt System</div>
        </div>""", unsafe_allow_html=True)
        
    with c3:
        st.markdown(f"""
        <div class="titan-card">
            <h4>Money Flow Matrix</h4>
            <h2 class="{mf_cls}">{mf_val:.2f}</h2>
            <div class="sub">Institutional Pressure</div>
        </div>""", unsafe_allow_html=True)
    ...
    with c4:
        ...
        <div class="titan-card">
            ...
            <h2 class="{rvol_cls}">{last['rvol']:.2f}x</h2>
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Titan Price Action", "Money Flow Matrix", f"Advanced Volume ({vol_metric})")
    )
    ...
    if not buys.empty:
        fig.add_trace(go.Scatter(x=buys['timestamp'], y=buys['low']*0.999, mode='markers', marker=dict(symbol='triangle-up', size=14, color='#00ffbb'), name='BUY'), row=1, col=1)
    sells = df[df['sell_signal']]
    if not sells.empty:
        fig.add_trace(go.Scatter(x=sells['timestamp'], y=sells['high']*1.001, mode='markers', marker=dict(symbol='triangle-down', size=14, color='#ff1155'), name='SELL'), row=1, col=1)
