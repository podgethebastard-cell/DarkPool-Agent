...
with st.sidebar:
    ...
    with st.expander("🌍 Feed Configuration", expanded=True):
        ...
        if st.checkbox("🔄 Auto-Pilot (60s)", st.session_state.auto):
            time_lib.sleep(60)
            st.rerun()
...
elif last["smc_buy"]: event = f"🏛️ SMC BUY: {st.session_state.sym} @ {last['close']}"
elif last["vec_state"] == 2 and prev["vec_state"] != 2: event = f"⚡ SUPERCONDUCTOR: {st.session_state.sym}"
...
with t5:
    ...
    <script>new TradingView.widget({{"width": "100%", "height": 500, "symbol": "{st.session_state.exch.upper()}:{tv_s}", "theme": "dark", "style": "1", "container_id": "tv"}});</script>
...
with t6:
    ...
    if st.button("🔱 CONSULT THE COUNCIL"):
        if OPENAI_AVAILABLE and st.session_state.ai_k:
            ...
            r = c.chat.completions.create(model="gpt-4o", messages=[{"role":"user", "content": f"Analyze: Flux {last['flux']}, Brain {last['br_trend']}, SMC {last['smc_buy']}. Give Bias and Entry."}])
